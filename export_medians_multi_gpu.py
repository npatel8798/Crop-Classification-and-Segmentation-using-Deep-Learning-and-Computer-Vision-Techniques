...
def process_patch(out_path, mode, num_buckets, root_coco_path, bands, padded_patch_height,
    ...
    for i in range(medians.shape[0]):
        for j in range(medians.shape[1]):
            for t in range(num_bins):
                cp.save(patch_dir / f'sub{str(sub_idx).rjust(subs_pad, "0")}_bin{str(t).rjust(bins_pad, "0")}', medians[i, j, t, :, :, :].astype(medians_dtype))
            sub_idx += 1
    ...
    for i in range(labels.shape[0]):
        for j in range(labels.shape[1]):
            cp.save(patch_dir / f'labels_sub{str(lbl_idx).rjust(lbl_pad, "0")}', labels[i, j, :, :].astype(label_dtype))
            lbl_idx += 1
...
def sliding_window_view(arr, window_shape, steps):
    ...
    Produce a view from a sliding, striding window over `arr`.
        ...
        view of `arr`, shape=([X, (...), Z], ..., [Wx, (...), Wz])
            Where X = (x - Wx) // Sx + 1
    '''
    in_shape = cp.array(arr.shape[-len(steps):])  # [x, (...), z]
    window_shape = cp.array(window_shape)  # [Wx, (...), Wz]
    steps = cp.array(steps)  # [Sx, (...), Sz]
    nbytes = arr.strides[-1]  # size (bytes) of an element in `arr`

    # number of per-byte steps to take to fill window
    window_strides = tuple(cp.cumprod(arr.shape[:0:-1])[::-1]) + (1,)
...
def get_medians(netcdf, start_bin, window, group_freq, bands,
    ...
    medians = cp.empty((len(bands), window, padded_patch_height, padded_patch_width), dtype=medians_dtype)
    ...
    for band_id, band in enumerate(bands):
        ...
        if expand_ratio != 1:
            band_data = cp.repeat(band_data, expand_ratio, axis=1)
            band_data = cp.repeat(band_data, expand_ratio, axis=2)

        # Add padding if needed
        if  (output_size[0] < band_data.shape[1]) or (output_size[1] < band_data.shape[2]):
            band_data = cp.pad(band_data,
                                pad_width=((0, 0), (pad_top, pad_bot), (pad_left, pad_right)),
                                mode='constant',
                                constant_values=0)

        medians[band_id, :, :, :] = cp.expand_dims(band_data, axis=0)
...
if __name__ == '__main__':
    ...
    medians_dtype = cp.float32
    label_dtype = cp.int16
